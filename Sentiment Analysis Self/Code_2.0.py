...
with open('Our Data.csv', 'r') as my_file:    
    ...
    with open('Cleaned_final_data_self.csv', 'w', newline='') as csvfile:
...
with open('Our Data.csv', 'r') as my_file:    
    ...
    with open('Review_sentiments_self.csv', 'w', newline='') as csvfile:
        filewriter = csv.writer(csvfile)
        for i in range(0, len(sentiments)):
            filewriter.writerow([reviews[i][1], sentiments[i]])
    csvfile.close()
my_file.close()
rating=0
#Getting sentiment score of individual business
with open('Review_sentiments_self.csv', 'r') as my_file:
    reader = csv.reader(my_file)
    reviews = list(reader)
    
    for review in reviews:
        rating=rev_score(review[1])
